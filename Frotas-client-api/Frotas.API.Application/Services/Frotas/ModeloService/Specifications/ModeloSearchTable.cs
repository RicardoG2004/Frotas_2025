...
{
  public class ModeloSearchTable : Specification<Modelo>
  {
    public ModeloSearchTable(List<TableFilter> filters, string? dynamicOrder = "")
    {
        ...
        foreach (TableFilter filter in filters)
        {
          switch (filter.Id.ToLower(System.Globalization.CultureInfo.CurrentCulture))
          {
            ...
            case "marca.nome":
              if (!string.IsNullOrWhiteSpace(filter.Value))
              {
                _ = Query.Where(x => x.Marca.Nome.Contains(filter.Value));
              }
              break;
            default:
              break;
