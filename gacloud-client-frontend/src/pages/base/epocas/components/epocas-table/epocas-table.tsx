...
import { useDeleteMultipleEpocas } from '../../queries/epocas-mutations'
import { columns } from './epocas-columns'
import { EpocasFilterControls } from './epocas-filter-controls'

type TEpocasTableProps = {
  epocas: EpocaDTO[]
  page: number
  pageSize: number
  total: number
...
export function EpocasTable({
  ...
  const deleteMultipleEpocasMutation = useDeleteMultipleEpocas()
  ...
  const { isDeleteModalOpen, setIsDeleteModalOpen, handleDeleteSelected } =
    useDeleteHandler(
      deleteMultipleEpocasMutation,
      'Épocas excluídas com sucesso',
      'Erro ao excluir épocas',
      state.selectedRows,
      undefined, // onSuccess callback
      'Algumas épocas foram excluídas com avisos', // partial success message
      (deletedIds: string[]) => {
      ...
      {epocas && (
        <>
          <DataTable
            ...
            FilterControls={EpocasFilterControls}
            toolbarActions={[
              {
                label: 'Remover',
          ...
          <AlertModal
            ...
            loading={deleteMultipleEpocasMutation.isPending}
            title='Remover Épocas'
            description='Tem certeza que deseja remover as épocas selecionadas?'
          />
        </>
      )}
    </>
