...
import { useCreateSepultura } from '@/pages/cemiterios/sepulturas/queries/sepulturas-mutations'
...
const sepulturaFormSchema = z.object({
  ...
  fundura1: z.boolean().default(false),
  fundura2: z.boolean().default(false),
  fundura3: z.boolean().default(false),
...
const SepulturaCreateForm = ({
  ...
  const createSepulturaMutation = useCreateSepultura()
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      talhaoId: '',
      sepulturaTipoId: '',
      sepulturaEstadoId: undefined,
      sepulturaSituacaoId: undefined,
      largura: 90, // predefined measure
      comprimento: 200, // predefined measure
      area: 18000, // calculated: 90 * 200
      profundidade: 180, // predefined measure - matches fundura2 recommendation
      fila: '',
      coluna: '',
      dataConcessao: undefined,
      dataInicioAluguer: undefined,
      dataFimAluguer: undefined,
      dataInicioReserva: undefined,
      dataFimReserva: undefined,
      dataConhecimento: undefined,
      numeroConhecimento: '',
      fundura1: true, // true because profundidade 180 >= 80
      fundura2: true, // true because profundidade 180 >= 160
      fundura3: false, // false because profundidade 180 < 220
      anulado: false,
      dataAnulacao: undefined,
      observacao: '',
      bloqueada: false,
      litigio: undefined,
      proprietarios: [],
  ...
  const getResponsibilityBadges = (proprietario: any) => {
    ...
    if (proprietario.isResponsavel) {
      badges.push({ label: 'Responsável', variant: 'secondary' as const })
    }
    if (proprietario.isResponsavelGuiaReceita) {
  ...
  useEffect(() => {
    if (isInitialized && hasFormData(formId)) {
      // Sort proprietarios by data (oldest first)
      if (formData && Array.isArray(formData.proprietarios)) {
        formData.proprietarios = formData.proprietarios.slice().sort((a, b) => {
      ...
      // If no saved data, use the pre-selected values with clean defaults
      form.reset({
        nome: initialNome || '',
        talhaoId: '',
        sepulturaTipoId: '',
  ...
  useEffect(() => {
    ...
    if (recommendedDepth > 0 && currentProfundidade !== recommendedDepth) {
      ...
      isProfundidadeFromFundura.current = true
      form.setValue('profundidade', predefinedMeasures.profundidade, {
        shouldValidate: true,
        shouldDirty: true,
      })
    }
  ...
  const predefinedMeasures = {
    comprimento: 200, // 2.00m in cm
    largura: 90, // 0.90m in cm (average of 0.80m to 1.00m)
    profundidade: 180, // 1.80m in cm (matches fundura2 recommendation)
  }

  // Add effect to set predefined measures when form is initialized
  useEffect(() => {
    if (isInitialized && !hasFormData(formId)) {
      // Set predefined measures
      form.setValue('comprimento', predefinedMeasures.comprimento)
      form.setValue('largura', predefinedMeasures.largura)
      form.setValue('profundidade', predefinedMeasures.profundidade)

      // Calculate and set area
      const area = predefinedMeasures.largura * predefinedMeasures.comprimento
      form.setValue('area', area)

      // Set fundura booleans based on profundidade (175cm >= 160, so fundura1 and fundura2 should be true)
      form.setValue('fundura1', true)
      form.setValue('fundura2', true)
      form.setValue('fundura3', false)
    }
  ...
  // Use the combined auto-selection hook for tipos (create form doesn't need return data)
  useAutoSelectionWithReturnData({
    ...
    returnDataKey: `return-data-${effectiveWindowId}-sepultura-tipo`, // Not used in create forms, but required by hook
  })

  // Use the combined auto-selection hook for talhões (create form doesn't need return data)
  useAutoSelectionWithReturnData({
    windowId: effectiveWindowId,
    instanceId,
    data: talhoesData,
    ...
    returnDataKey: `return-data-${effectiveWindowId}-talhao`, // Not used in create forms, but required by hook
  })

  // Use the combined auto-selection hook for proprietarios (create form doesn't need return data)
  useAutoSelectionWithReturnData({
    windowId: effectiveWindowId,
    instanceId,
    data: proprietariosData,
    setValue: (value: string) => {
      // Find the current proprietarios array and update the last empty proprietario
      const currentProprietarios = form.getValues('proprietarios') || []

      // Find the last proprietario that has an empty proprietarioId
      const lastEmptyIndex = currentProprietarios.findIndex((p, index) => {
        // Check if this is the last proprietario and it has an empty ID
        return index === currentProprietarios.length - 1 && !p.proprietarioId
      })

      if (lastEmptyIndex !== -1) {
        // Update the existing empty proprietario
        form.setValue(`proprietarios.${lastEmptyIndex}.proprietarioId`, value)
      } else {
        // If no empty proprietario found, add a new one
        const newProprietario = {
          id: undefined,
          proprietarioId: value,
          cemiterioId: undefined,
          entidadeId: undefined,
          data: new Date(),
          ativo: true,
          isProprietario: currentProprietarios.length === 0, // true if first, false if not first
          isResponsavel: currentProprietarios.length === 0, // true if first, false if not first
          isResponsavelGuiaReceita: currentProprietarios.length === 0, // true if first, false if not first
    ...
    returnDataKey: `return-data-${effectiveWindowId}-proprietario`, // Not used in create forms, but required by hook
  })
  ...
  const onSubmit = async (values: SepulturaFormSchemaType) => {
    try {
      ...
      // Map proprietarios data to match API structure
      const proprietarios = values.proprietarios?.map((p) => {
        const dto: any = {
          sepulturaId: undefined, // Will be set by the API
      ...
      const response = await createSepulturaMutation.mutateAsync({
        ...values,
        dataConcessao: toDatabaseDate(values.dataConcessao),
        dataInicioAluguer: toDatabaseDate(values.dataInicioAluguer),
        dataFimAluguer: toDatabaseDate(values.dataFimAluguer),
        dataInicioReserva: toDatabaseDate(values.dataInicioReserva),
        dataFimReserva: toDatabaseDate(values.dataFimReserva),
        dataConhecimento: toDatabaseDate(values.dataConhecimento),
        dataAnulacao: toDatabaseDate(values.dataAnulacao),
        proprietarios,
      })

      const result = handleApiResponse(
        response,
        'Sepultura criada com sucesso',
        'Erro ao criar sepultura',
        'Sepultura criada com avisos'
      )

      if (result.success) {
        const newSepulturaId = response.info.data

        // Set return data for parent window if this window was opened from another window
        if (effectiveWindowId) {
          setEntityReturnDataWithToastSuppression(
            effectiveWindowId,
            { id: newSepulturaId, nome: values.nome },
            'sepultura',
            setWindowReturnData,
      ...
      toast.error(handleApiError(error, 'Erro ao criar sepultura'))
    } finally {
      updateFormState(formId, (state) => ({
        ...state,
        isSubmitting: false,
      ...
      <Form {...form}>
        ...
        >
          ...
          >
            ...
            <TabsContent value='proprietarios'>
              <div className='space-y-6'>
                ...
                {/* Proprietarios List */}
                <div className='space-y-4'>
                  {form.watch('proprietarios')?.map((proprietario, index) => {
                              ...
                              <div className='flex-1'>
                                ...
                                {!expandedProprietarios.has(index) && (
                                  <div className='flex flex-wrap gap-1 mt-2'>
                                    {getResponsibilityBadges(proprietario).map(
                                      (badge, badgeIndex) => (
                                        <Badge
                                          key={badgeIndex}
                            ...
                            <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                              ...
                              <FormField
                                ...
                                render={({ field }) => {
                                  const validation = validateFracaoPercentages()
                                  const isInvalid =
                                    !validation.isValid && validation.total > 0
                  ...
                  {(!form.watch('proprietarios') ||
                    form.watch('proprietarios')?.length === 0) && (
                    <Card className='border-dashed border-2 border-muted-foreground/20'>
                      <CardContent className='flex flex-col items-center justify-center py-12 text-center'>
                        ...
                        <p className='text-sm text-muted-foreground mb-4 max-w-sm'>
                          Adicione proprietários para associar a esta sepultura
          ...
          <div className='flex flex-col justify-end space-y-2 pt-4 md:flex-row md:space-x-4 md:space-y-0'>
            ...
            <Button
              type='submit'
              disabled={createSepulturaMutation.isPending}
              className='w-full md:w-auto'
            >
              {createSepulturaMutation.isPending ? 'A criar...' : 'Criar'}
            </Button>
          </div>
        </form>
      </Form>
