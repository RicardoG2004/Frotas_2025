...
import { useDeleteMultipleProprietarios } from '../../queries/proprietarios-mutations'
import { columns } from './proprietarios-columns'
import { ProprietariosFilterControls } from './proprietarios-filter-controls'

type TProprietariosTableProps = {
  proprietarios: ProprietarioDTO[]
  page: number
  pageSize: number
  total: number
  pageCount: number
...
export function ProprietariosTable({
  proprietarios,
  pageCount,
  total,
  onFiltersChange,
  onPaginationChange,
  onSortingChange,
}: TProprietariosTableProps) {
  const deleteMultipleProprietariosMutation = useDeleteMultipleProprietarios()
  ...
  const { isDeleteModalOpen, setIsDeleteModalOpen, handleDeleteSelected } =
    useDeleteHandler(
      deleteMultipleProprietariosMutation,
      'Proprietários excluídos com sucesso',
      'Erro ao excluir proprietários',
      state.selectedRows,
      undefined, // onSuccess callback
      'Alguns proprietários foram excluídos com avisos', // partial success message
      (deletedIds: string[]) => {
      ...
      {proprietarios && (
        <>
          <DataTable
            ...
            FilterControls={ProprietariosFilterControls}
            toolbarActions={[
              {
                label: 'Remover',
          ...
          <AlertModal
            ...
            loading={deleteMultipleProprietariosMutation.isPending}
            title='Remover Proprietários'
            description='Tem certeza que deseja remover os proprietários selecionados?'
          />
        </>
      )}
    </>
