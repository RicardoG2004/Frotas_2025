...
import { useCreateSepulturaTipoDescricao } from '@/pages/cemiterios/sepulturas-tipos-descricoes/queries/sepulturas-tipos-descricoes-mutations'
...
interface SepulturaTipoDescricaoCreateFormProps {
  modalClose: () => void
  preSelectedCemiterioId?: string
  initialDescricao?: string
  onSuccess?: (newSepulturaTipoDescricao: {
    id: string
    descricao: string
  }) => void
...
const SepulturaTipoDescricaoCreateForm = ({
  modalClose,
  preSelectedCemiterioId,
  initialDescricao,
  onSuccess,
  shouldCloseWindow = true,
}: SepulturaTipoDescricaoCreateFormProps) => {
  const location = useLocation()
  const navigate = useNavigate()
  const windowId = useCurrentWindowId()
  const searchParams = new URLSearchParams(location.search)
  const instanceId = searchParams.get('instanceId') || 'default'
  const formId = `sepultura-tipo-descricao-${instanceId}`
  ...
  const createSepulturaTipoDescricaoMutation = useCreateSepulturaTipoDescricao()
  ...
  useEffect(() => {
    ...
    } else if (preSelectedCemiterioId || initialDescricao) {
  ...
  const onSubmit = async (values: SepulturaTipoDescricaoFormSchemaType) => {
    try {
      ...
      const response =
        await createSepulturaTipoDescricaoMutation.mutateAsync(finalValues)
      const result = handleApiResponse(
        response,
        'Descrição de tipo de sepultura criada com sucesso',
        'Erro ao criar descrição de tipo de sepultura',
        'Descrição de tipo de sepultura criada com avisos'
      )
      ...
      toast.error(
        handleApiError(error, 'Erro ao criar descrição de tipo de sepultura')
      )
    } finally {
      updateFormState(formId, (state) => ({
        ...state,
  ...
  return (
    <div className='space-y-4'>
      <Form {...form}>
        <form
          id='sepultura-tipo-descricao-create-form'
        ...
        >
          ...
          <div className='flex flex-col justify-end space-y-2 pt-4 md:flex-row md:space-x-4 md:space-y-0'>
            ...
            <Button
              type='submit'
              disabled={createSepulturaTipoDescricaoMutation.isPending}
              className='w-full md:w-auto'
            >
              {createSepulturaTipoDescricaoMutation.isPending
                ? 'A criar...'
                : 'Criar'}
            </Button>
          </div>
...
export { SepulturaTipoDescricaoCreateForm }
