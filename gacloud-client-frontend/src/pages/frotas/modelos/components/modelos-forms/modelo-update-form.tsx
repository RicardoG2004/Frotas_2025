...
import { UpdateModeloDTO } from '@/types/dtos/frotas/modelos.dtos'
...
import { useUpdateModelo } from '@/pages/frotas/modelos/queries/modelos-mutations'
...
interface ModeloUpdateFormProps {
  modeloId: string
  modeloData: {
    nome: string
    marcaId: string
...
export const ModeloUpdateForm = ({
  modeloId,
  modeloData,
  modalClose,
}: ModeloUpdateFormProps) => {
  ...
  useEffect(() => {
    const subscription = form.watch((value) => {
      ...
      setFormState(formId, {
        formData: value as ModeloFormSchemaType,
        isDirty: hasChanges,
      ...
      if (effectiveWindowId) {
        ...
        const newTitle = value.nome || 'Modelo'
        updateUpdateWindowTitle(effectiveWindowId, newTitle, updateWindowState)
      }
    })
    return () => subscription.unsubscribe()
  }, [form, effectiveWindowId, formId, formData, modeloData])
  ...
  const onSubmit = async (values: ModeloFormSchemaType) => {
    try {
      ...
      const requestData: UpdateModeloDTO = {
        nome: values.nome,
        marcaId: values.marcaId,
      }

      const response = await updateModeloMutation.mutateAsync({
        id: modeloId,
        data: requestData,
      })
      const result = handleApiResponse(
        ...
        'Modelo atualizado com avisos'
      )

      if (result.success) {
        console.log('Modelo updated successfully:', result.data)

        // Set return data for parent window if this window was opened from another window
        if (effectiveWindowId) {
          setEntityReturnDataWithToastSuppression(
            effectiveWindowId,
            { id: modeloId, nome: values.nome },
            'modelo',
            setWindowReturnData,
      ...
      toast.error(handleApiError(error, 'Erro ao atualizar modelo'))
    } finally {
      updateFormState(formId, (state) => ({
        ...state,
        isSubmitting: false,
