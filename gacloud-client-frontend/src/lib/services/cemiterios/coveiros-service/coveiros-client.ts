...
export class CoveiroClient extends BaseApiClient {
  public async getCoveirosPaginated(
    ...
    const cacheKey = this.getCacheKey(
      'POST',
      '/client/cemiterios/coveiros/paginated',
      params
    )
    return this.withCache(cacheKey, () =>
      this.withRetry(async () => {
        try {
          ...
          >(state.URL, '/client/cemiterios/coveiros/paginated', params)
  ...
  public async getCoveiros(): Promise<ResponseApi<GSResponse<CoveiroDTO[]>>> {
    const cacheKey = this.getCacheKey('GET', '/client/cemiterios/coveiros')
    return this.withCache(cacheKey, () =>
      this.withRetry(async () => {
        try {
          const response = await this.httpClient.getRequest<
            GSResponse<CoveiroDTO[]>
          >(state.URL, '/client/cemiterios/coveiros')
    ...
    return this.withCache(cacheKey, () =>
      this.withRetry(async () => {
        try {
          const response = await this.httpClient.getRequest<
            GSResponse<CoveiroDTO>
          >(state.URL, `/client/cemiterios/coveiros/${id}`)
    ...
    return this.withRetry(async () => {
      try {
        const response = await this.httpClient.postRequest<
          CreateCoveiroDTO,
          GSResponse<string>
        >(state.URL, '/client/cemiterios/coveiros', data)
    ...
    return this.withRetry(async () => {
      try {
        const response = await this.httpClient.putRequest<
          UpdateCoveiroDTO,
          GSResponse<string>
        >(state.URL, `/client/cemiterios/coveiros/${id}`, data)
    ...
    return this.withRetry(async () => {
      try {
        const response = await this.httpClient.deleteRequestWithBody<
          { ids: string[] },
          GSGenericResponse
        >(state.URL, '/client/cemiterios/coveiros/bulk', { ids: ids })
