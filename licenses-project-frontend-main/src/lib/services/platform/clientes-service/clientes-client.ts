...
export class ClientesClient extends BaseApiClient {
  ...
  public async getClientesByAplicacao(
    aplicacaoId: string
  ): Promise<ResponseApi<GSResponse<ClienteDTO[]>>> {
    const cacheKey = this.getCacheKey(
      'GET',
      `/api/clientes/by-aplicacao/${aplicacaoId}`
    )
    return this.withCache(cacheKey, () =>
      this.withRetry(async () => {
        try {
          const response = await this.httpClient.getRequest<
            GSResponse<ClienteDTO[]>
          >(`/api/clientes/by-aplicacao/${aplicacaoId}`)
          ...
          throw new ClienteError(
            'Falha ao obter clientes por aplicação',
            undefined,
            error
          )
        }
