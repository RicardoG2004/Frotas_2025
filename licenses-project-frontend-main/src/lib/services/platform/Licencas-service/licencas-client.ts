...
export class LicencasClient extends BaseApiClient {
  ...
  public async getLicencasByLicencaId(
    licencaId: string
  ): Promise<ResponseApi<GSResponse<LicencaDTO[]>>> {
    const cacheKey = this.getCacheKey(
      'GET',
      `/api/licencas/${licencaId}/by-cliente`
    )
    return this.withCache(cacheKey, () =>
      this.withRetry(async () => {
        try {
          ...
          throw new LicencaError(
            'Falha ao obter licenças do cliente pela licença',
            undefined,
            error
          )
        }
  ...
  public async downloadUpdaterConfig(
    licenseIds: string[]
  ): Promise<{ blob: Blob; filename: string }> {
    return this.withRetry(async () => {
      try {
        const result = await this.httpClient.downloadFilePostRequest<{
          Ids: string[]
        }>('/api/Licencas/download-updater-config', { Ids: licenseIds })

        return result
      } catch (error) {
        throw new LicencaError(
          'Falha ao descarregar configuração do atualizador',
          undefined,
          error
        )
      }
  ...
  public async downloadFrontendConfig(
    licenseId: string
  ): Promise<{ blob: Blob; filename: string }> {
    return this.withRetry(async () => {
      try {
        const result = await this.httpClient.downloadFilePostRequest<{
          Id: string
        }>('/api/Licencas/download-frontend-config', { Id: licenseId })

        return result
      } catch (error) {
        throw new LicencaError(
          'Falha ao descarregar configuração do frontend',
          undefined,
          error
        )
      }
  ...
  public async downloadApiConfig(
    licenseId: string
  ): Promise<{ blob: Blob; filename: string }> {
    return this.withRetry(async () => {
      try {
        const result = await this.httpClient.downloadFilePostRequest<{
          Id: string
        }>('/api/Licencas/download-api-config', { Id: licenseId })

        return result
      } catch (error) {
        throw new LicencaError(
          'Falha ao descarregar configuração da API',
          undefined,
          error
        )
      }
