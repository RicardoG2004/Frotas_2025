...
import { useCreateAppUpdate } from '@/pages/application/app-updates/queries/app-updates-mutations'
...
interface AppUpdateCreateFormProps {
  modalClose: () => void
  aplicacaoId: string
}

const AppUpdateCreateForm = ({
  modalClose,
  aplicacaoId,
}: AppUpdateCreateFormProps) => {
  const createUpdateMutation = useCreateAppUpdate()
        ...
        versao: z
          ...
          .refine(
            (val) => {
              ...
              if (!version) return true // If no current version, allow any
  ...
  const form = useForm<AppUpdateFormSchemaType>({
    ...
    defaultValues: {
      ...
      ativo: true,
      versaoMinima: undefined,
      tipoUpdate: UpdateType.Both,
      notasAtualizacao: '',
      clienteIds: [],
    },
  })
  ...
  const onSubmit = async (values: AppUpdateFormSchemaType) => {
    try {
      const response = await createUpdateMutation.mutateAsync({
        versao: values.versao,
        descricao: values.descricao,
        dataLancamento: format(values.dataLancamento, "yyyy-MM-dd'T'HH:mm:ss"),
        ativo: values.ativo,
        obrigatorio: true,
      ...
      if (response.info.succeeded) {
        ...
        toast.error(getErrorMessage(response, 'Erro ao criar atualização'))
      }
    } catch (error) {
      toast.error(handleApiError(error, 'Erro ao criar atualização'))
    }
  }
        ...
        >
          ...
          <div className='flex flex-col justify-end space-y-2 pt-4 md:flex-row md:space-x-4 md:space-y-0'>
            ...
            <Button
              ...
              className='w-full md:w-auto'
            >
              {createUpdateMutation.isPending ? 'A criar...' : 'Criar'}
            </Button>
          </div>
        </form>
      </Form>
...
export default AppUpdateCreateForm
