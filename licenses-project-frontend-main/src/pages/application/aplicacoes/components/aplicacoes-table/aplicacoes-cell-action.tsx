...
import { AppWindow, Edit, Trash, Package } from 'lucide-react'
...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const handleViewUpdates = (aplicacaoId: string) => {
    navigate(`/administracao/aplicacoes/${aplicacaoId}/updates`)
  }
  ...
  return (
    <>
      ...
      <div className='flex items-center gap-2'>
        ...
        <Button
          onClick={() => handleViewUpdates(data.id || '')}
          variant='ghost'
          className='h-8 w-8 p-0'
          title='Gerir Atualizações'
        >
          <Package className='h-4 w-4' />
          <span className='sr-only'>Gerir Atualizações</span>
        </Button>
      </div>
    </>
  )
