...
export const useGetClientesByAplicacao = (aplicacaoId: string) => {
  return useQuery({
    queryKey: ['clientes-by-aplicacao', aplicacaoId],
    queryFn: async () => {
      const response =
        await ClientesService('clientes').getClientesByAplicacao(aplicacaoId)
      const clientes = response.info.data || []
      // Sort by nome
