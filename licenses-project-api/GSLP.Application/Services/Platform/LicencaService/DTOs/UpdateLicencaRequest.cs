...
{
    public class UpdateLicencaRequest : IDto
    {
    ...
    public class UpdateLicencaValidator : AbstractValidator<UpdateLicencaRequest>
    {
        public UpdateLicencaValidator()
        {
            _ = RuleFor(x => x.Nome)
            ...
            //     .NotEmpty().WithMessage("O ID da aplicação é obrigatório.")
            //     .Must(GSLPHelpers.BeAValidGuid).WithMessage("O ID da aplicação deve ser válido")
            //     .Must(GSLPHelpers.BeNotEmptyGuid).WithMessage("O ID da aplicação não pode estar vazio");

            // _ = RuleFor(x => x.ClienteId)
            //     .NotEmpty().WithMessage("O ID do cliente é obrigatório.")
            //     .Must(GSLPHelpers.BeAValidGuid).WithMessage("O ID do cliente deve ser válido")
            //     .Must(GSLPHelpers.BeNotEmptyGuid).WithMessage("O ID do cliente não pode estar vazio");
        }
    }
}
