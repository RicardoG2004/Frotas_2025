...
namespace GSLP.Application.Services.Application.ModuloService
{
    public class ModuloService : BaseService, IModuloService
    {
        ...
        {
            ModuloMatchName moduloMatchName = new(request.Nome); // ardalis specification
            bool ModuloExists = await _repository.ExistsAsync<Modulo, Guid>(moduloMatchName);
            if (ModuloExists)
            {
                return Response<Guid>.Fail("Já existe um módulo com o nome fornecido");
            }
