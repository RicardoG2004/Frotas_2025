    ...
    public MappingProfiles()
    {
      ...
      _ = CreateMap<ApplicationUser, UserDto>()
        ...
        .ForMember(
          ...
          opt =>
            opt.MapFrom(src => src.PerfisUtilizadores.Select(p => p.PerfilId.ToConsistentString()))
        )
        .ForMember(
          dest => dest.LicencaId,
          opt =>
            opt.MapFrom(src =>
              src.LicencasUtilizadores != null && src.LicencasUtilizadores.Any()
                ? src.LicencasUtilizadores.FirstOrDefault().LicencaId.ToConsistentString()
                : null
            )
        );
      ...
      _ = CreateMap<Funcionalidade, PerfilFuncionalidadeTreeDTO>()
        ...
        .ForMember(
          dest => dest.Estado,
          opt => opt.MapFrom(src => src.PerfisFuncionalidades.All(pf => pf.AuthVer) ? 2 : 1)
      ...
      _ = CreateMap<Licenca, LicencaDTO>()
        .ForMember(
          dest => dest.APIKey,
          opt => opt.MapFrom(src => src.LicencaAPIKey != null ? src.LicencaAPIKey.APIKey : null)
        );
      ...
      _ = CreateMap<Licenca, LicencaModulosFuncionalidadesDTO>()
        ...
        .ForMember(
          ...
          opt =>
            opt.MapFrom(src =>
              src.LicencasFuncionalidades.Select(lf => new LicencaFuncionalidadeNomeDTO
                {
      ...
      _ = CreateMap<PerfilFuncionalidade, PerfilFuncionalidadeDTO>()
        .ForMember(dest => dest.FuncionalidadeId, opt => opt.MapFrom(src => src.FuncionalidadeId))
        .ForMember(
      ...
      _ = CreateMap<LicencaUtilizador, LicencaUtilizadorDTOFlat>()
        ...
        .ForMember(dest => dest.ImageUrl, opt => opt.MapFrom(src => src.Utilizador.ImageUrl))
        .ForMember(dest => dest.FirstName, opt => opt.MapFrom(src => src.Utilizador.FirstName))
        .ForMember(dest => dest.LastName, opt => opt.MapFrom(src => src.Utilizador.LastName))
        .ForMember(dest => dest.Email, opt => opt.MapFrom(src => src.Utilizador.Email))
        .ForMember(dest => dest.IsActive, opt => opt.MapFrom(src => src.Utilizador.IsActive))
        .ForMember(dest => dest.PhoneNumber, opt => opt.MapFrom(src => src.Utilizador.PhoneNumber))
        .ForMember(dest => dest.RoleId, opt => opt.MapFrom(src => src.Utilizador.RoleId))
        .ForMember(dest => dest.ClienteId, opt => opt.MapFrom(src => src.Utilizador.ClienteId))
        // .ForMember(dest => dest.Cliente, opt => opt.MapFrom(src => src.Utilizador.Cliente))
        .ForMember(dest => dest.CreatedOn, opt => opt.MapFrom(src => src.Utilizador.CreatedOn));
      ...
      // appupdate mappings
      _ = CreateMap<AppUpdate, AppUpdateDTO>()
        .ForMember(dest => dest.Id, opt => opt.MapFrom(src => src.Id.ToString().ToLowerInvariant()))
        .ForMember(
          dest => dest.AplicacaoId,
          opt => opt.MapFrom(src => src.AplicacaoId.ToString().ToLowerInvariant())
        )
        .ForMember(
          dest => dest.ClienteIds,
          opt =>
            opt.MapFrom(src =>
              src.AppUpdatesClientes != null && src.AppUpdatesClientes.Any()
                ? src
                  .AppUpdatesClientes.Select(ac => ac.ClienteId.ToString().ToLowerInvariant())
                  .ToList()
                : null
            )
        );
      _ = CreateMap<CreateAppUpdateRequest, AppUpdate>()
        .ForMember(dest => dest.AppUpdatesClientes, opt => opt.Ignore());
      _ = CreateMap<UpdateAppUpdateRequest, AppUpdate>()
        .ForMember(dest => dest.AppUpdatesClientes, opt => opt.Ignore())
        .ForMember(dest => dest.Obrigatorio, opt => opt.Ignore()); // Obrigatorio is always set to true in service layer
