...
namespace GSLP.Infrastructure.Auth.JWT
{
  public class ClientTokenService : IClientTokenService
  {
    ...
    public async Task<Response<ClientTokenResponse>> GetClientTokenAsync(ClientTokenRequest request)
    {
      ...
      // Check for available updates, using client-provided app version if available
      // Skip update check in development mode
      DTOs.UpdateInfo updateInfo = _environment.IsDevelopment()
        ? new DTOs.UpdateInfo { HasUpdate = false }
        : await CheckForUpdatesAsync(
            licencaAPIKey.Licenca.AplicacaoId,
            request.AppVersion,
    ...
    public async Task<Response<ClientTokenResponse>> RefreshClientTokenAsync(string refreshToken)
    {
      ...
      // Check for available updates (no client version on refresh for now, falls back to DB version)
      // Skip update check in development mode
      DTOs.UpdateInfo updateInfo = _environment.IsDevelopment()
        ? new DTOs.UpdateInfo { HasUpdate = false }
        : await CheckForUpdatesAsync(
    ...
    private async Task<DTOs.UpdateInfo> CheckForUpdatesAsync(
      Guid aplicacaoId,
    ...
    )
    {
      try
      {
        // Get current application version from the license's application
        var aplicacao = await _dbContext.Aplicacoes.FindAsync(aplicacaoId);
        if (aplicacao == null)
        {
          return new DTOs.UpdateInfo { HasUpdate = false };
        }

        // Prefer client-sent version when available; otherwise fall back to DB-stored version
        string currentVersion = !string.IsNullOrWhiteSpace(clientVersion)
          ? clientVersion
          : aplicacao.Versao ?? "0.0.0";

        // Check for updates
        CheckUpdateRequest checkRequest = new()
        {
        ...
        Response<CheckUpdateResponse> updateResponse =
          await _appUpdateService.CheckForUpdateResponseAsync(checkRequest);

        if (updateResponse.Succeeded && updateResponse.Data?.TemUpdate == true)
        {
          var updates = updateResponse.Data.UpdatesDisponiveis?
            .Select(u => new DTOs.UpdateItem
            {
              UpdateId = u.Id,
              Version = u.Versao,
              UpdateType = (int)u.TipoUpdate,
              IsMandatory = u.Obrigatorio,
              Description = u.Descricao,
              ReleaseNotes = u.NotasAtualizacao,
              ReleaseDate = u.DataLancamento,
            })
            .ToList() ?? new List<DTOs.UpdateItem>();

          return new DTOs.UpdateInfo
          {
            HasUpdate = true,
            UpdateId = updateResponse.Data.UpdateDisponivel?.Id,
            UpdateVersion = updateResponse.Data.UpdateDisponivel?.Versao,
            UpdateType = (int)(updateResponse.Data.UpdateDisponivel?.TipoUpdate ?? 0),
            IsMandatory = updateResponse.Data.Obrigatorio,
            UpdateDescription = updateResponse.Data.UpdateDisponivel?.Descricao,
            ReleaseNotes = updateResponse.Data.UpdateDisponivel?.NotasAtualizacao,
            ReleaseDate = updateResponse.Data.UpdateDisponivel?.DataLancamento,
            Updates = updates,
          };
        }
      ...
      {
        // If update check fails, don't block login - just return no update
        return new DTOs.UpdateInfo { HasUpdate = false };
      }
    }
