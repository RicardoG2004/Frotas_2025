...
namespace GSLP.Application.Services.Platform.LicencaService
{
    public interface ILicencaService : ITransientService
    {
        ...
        Task<Response<IEnumerable<LicencaDTO>>> GetLicencasByLicencaIdResponseAsync(Guid licencaId);
        Task<Response<List<Guid>>> DeleteLicencasAsync(List<Guid> ids);
        Task<Response<Guid>> UpdateLicencaVersaoResponseAsync(Guid id, string versao);
        Task<Response<UpdaterConfigurationDTO>> GetUpdaterConfigurationResponseAsync(List<Guid> licencaIds);
        Task<Response<FrontendConfigDTO>> GetFrontendConfigResponseAsync(Guid licencaId);
        Task<Response<ApiConfigDTO>> GetApiConfigResponseAsync(Guid licencaId);
