...
namespace GSLP.Application.Services.Platform.ClienteService
{
    public class ClienteService : BaseService, IClienteService
    {
        ...
        // get all clients that have an active licenca for the specified aplicacao
        public async Task<Response<IEnumerable<ClienteDTO>>> GetClientesByAplicacaoResponseAsync(
            Guid aplicacaoId
        )
        {
            try
            {
                ...
                var aplicacao = await _repository.GetByIdAsync<Domain.Entities.Catalog.Application.Aplicacao, Guid>(aplicacaoId);
                if (aplicacao == null)
                {
                    return Response<IEnumerable<ClienteDTO>>.Fail("Aplicação não encontrada");
                }

                // Get all active licencas for this aplicacao
                var licencaSpec = new LicencasByAplicacaoId(aplicacaoId);
                var licencas = await _repository.GetListAsync<Licenca, Guid>(licencaSpec);

                // Extract unique clienteIds
                var clienteIds = licencas.Select(l => l.ClienteId).Distinct().ToList();

                if (!clienteIds.Any())
                {
                    // No licencas found, return empty list
                    return Response<IEnumerable<ClienteDTO>>.Success(new List<ClienteDTO>());
                }

                // Get all clientes for these IDs by querying entities directly
                var allClientes = await _repository.GetListAsync<Cliente, Guid>();
                var clientes = allClientes.Where(c => clienteIds.Contains(c.Id)).ToList();

                // Map to DTOs
                var clienteDtos = clientes.Select(c => _mapper.Map<ClienteDTO>(c)).ToList();

                return Response<IEnumerable<ClienteDTO>>.Success(clienteDtos);
            }
            catch (Exception ex)
            {
                return Response<IEnumerable<ClienteDTO>>.Fail(ex.Message);
            }
        }
