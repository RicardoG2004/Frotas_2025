...
namespace GSLP.Application.Services.Application.FuncionalidadeService
{
    public class FuncionalidadeService : BaseService, IFuncionalidadeService
    {
        ...
        )
        {
            ...
            FuncionalidadeMatchNameAndModuloId funcionalidadeMatchNomeAndModuloId = new(request.Nome, moduloGuid); // ardalis specification
            bool FuncionalidadeExists = await _repository.ExistsAsync<Funcionalidade, Guid>(
                funcionalidadeMatchNomeAndModuloId
            );
            if (FuncionalidadeExists)
            {
                return Response<Guid>.Fail("Já existe uma funcionalidade com o nome fornecido para este módulo");
            }
        ...
        )
        {
            ...
            // Check if another funcionalidade with the same name exists in the same modulo (excluding current funcionalidade)
            FuncionalidadeMatchNameAndModuloIdExcludingId funcionalidadeMatchNomeAndModuloIdExcludingId = new(request.Nome, moduloGuid, id);
            bool FuncionalidadeExists = await _repository.ExistsAsync<Funcionalidade, Guid>(
                funcionalidadeMatchNomeAndModuloIdExcludingId
            );
            if (FuncionalidadeExists)
            {
                return Response<Guid>.Fail("Já existe uma funcionalidade com o nome fornecido para este módulo");
