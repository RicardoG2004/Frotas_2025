...
import { useUpdateFse } from '../../queries/fses-mutations'
...
const FseUpdateForm = ({
  ...
  const formId = `fse-update-${instanceId}`
  ...
  const updateFseMutation = useUpdateFse()
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      numContribuinte: initialData.numContribuinte || '',
      ...
      enderecoHttp: initialData.enderecoHttp || '',
  ...
  }, [formData, isInitialized, formId, hasFormData, defaultValues])

  useEffect(() => {
    const subscription = form.watch((value) => {
      const hasChanges = detectUpdateFormChanges(value, defaultValues)
      ...
      if (JSON.stringify(value) !== JSON.stringify(formData)) {
        ...
        if (effectiveWindowId) {
          ...
          if (value.nome && value.nome !== formData?.nome) {
            updateUpdateWindowTitle(
              effectiveWindowId,
              value.nome || 'Fornecedor Serviços Externos',
              updateWindowState
            )
          }
        }
  ...
  const onSubmit = async (values: FseFormSchemaType) => {
    try {
      ...
      const requestData: UpdateFseDTO = {
        nome: values.nome,
      ...
      const response = await updateFseMutation.mutateAsync({
        id: fseId,
        data: requestData,
      })
      const result = handleApiResponse(
        response,
        'Fornecedor Serviços Externos atualizado com sucesso',
        'Erro ao atualizar fornecedor serviços externos',
        'Fornecedor Serviços Externos atualizado com avisos'
      )
      ...
      toast.error(handleApiError(error, 'Erro ao atualizar fornecedor serviços externos'))
    } finally {
      updateFormState(formId, (state) => ({
        ...state,
        isSubmitting: false,
        ...
        >
          ...
          <div className='flex items-center justify-end gap-3 pt-4 border-t'>
            ...
            <Button
              type='submit'
              form='fseUpdateForm'
              disabled={updateFseMutation.isPending}
            >
              {updateFseMutation.isPending ? 'A atualizar...' : 'Atualizar'}
            </Button>
          </div>
        </form>
      </Form>
