...
import { useUpdateFuncionario } from '../../queries/funcionarios-mutations'
...
interface FuncionarioUpdateFormProps {
  modalClose: () => void
...
const FuncionarioUpdateForm = ({
  modalClose,
  funcionarioId,
  initialData,
}: FuncionarioUpdateFormProps) => {
  ...
  }, [formId, hasBeenVisited, resetFormState, hasFormData, initialData, setFormState, windowId])

  useEffect(() => {
    if (isInitialized && hasFormData(formId)) {
      form.reset(formData as FuncionarioFormValues)
      if ((formData as FuncionarioFormValues)?.nome && windowId) {
        updateUpdateWindowTitle(
          windowId,
          (formData as FuncionarioFormValues).nome,
          updateWindowState
        )
      }
    } else if (initialData) {
      form.reset(initialData)
      if (initialData.nome && windowId) {
        updateUpdateWindowTitle(windowId, initialData.nome, updateWindowState)
      }
    }
  }, [formData, isInitialized, formId, hasFormData, initialData, form, windowId, updateWindowState])
  ...
  useEffect(() => {
    const subscription = form.watch((value) => {
      ...
      if (JSON.stringify(value) !== JSON.stringify(formData)) {
        ...
        if (value.nome && value.nome !== (formData as FuncionarioFormValues)?.nome) {
  ...
  useAutoSelectionWithReturnData({
    ...
    returnDataKey: `return-data-${windowId}-cargo`,
  })
  ...
  useAutoSelectionWithReturnData({
    ...
    returnDataKey: `return-data-${windowId}-delegacao`,
  })
  ...
  const onSubmit = async (values: FuncionarioFormValues) => {
    try {
      ...
      const response = (await updateFuncionarioMutation.mutateAsync({
        id: funcionarioId,
        data: {
          Nome: values.nome,
      ...
      const result = handleApiResponse(
        ...
        'Funcionário atualizado com avisos'
      )
      ...
      toast.error(handleApiError(error, 'Erro ao atualizar funcionário'))
    } finally {
      updateFormState(formId, (state) => ({
        ...state,
        isSubmitting: false,
                  ...
                  <div className='flex items-center gap-3'>
                    ...
                    <div>
                      ...
                      <p className='text-sm text-muted-foreground mt-1'>
                        Atualize a identidade e contactos do funcionário
                      </p>
                    </div>
                  </div>
                </CardHeader>
                ...
                <CardHeader className='pb-4'>
                  <div className='flex items-center gap-3'>
                    ...
                    <div>
                      ...
                      <p className='text-sm text-muted-foreground mt-1'>
                        Atualize a localização e delegação associada
                      </p>
                    </div>
                  </div>
                </CardHeader>
...
export default FuncionarioUpdateForm
