...
import { UpdateFornecedorDTO } from '@/types/dtos/frotas/fornecedores.dtos'
...
import { useUpdateFornecedor } from '../../queries/fornecedores-mutations'
...
interface FornecedorUpdateFormProps {
  modalClose: () => void
  fornecedorId: string
  initialData: FornecedorFormSchemaType
  onSuccess?: (updatedFornecedor: { id: string; nome: string }) => void
...
const FornecedorUpdateForm = ({
...
}: FornecedorUpdateFormProps) => {
  const location = useLocation()
  const navigate = useNavigate()
  const windowId = useCurrentWindowId()
  const searchParams = new URLSearchParams(location.search)
  const instanceId = searchParams.get('instanceId') || 'default'
  const formId = `fornecedor-update-${instanceId}`
  ...
  const onSubmit = async (values: FornecedorFormSchemaType) => {
    try {
      ...
      const requestData: UpdateFornecedorDTO = {
        nome: values.nome,
      ...
      const response = await updateFornecedorMutation.mutateAsync({
        id: fornecedorId,
        data: requestData,
      })
      const result = handleApiResponse(
        ...
        'Fornecedor atualizado com avisos'
      )
      ...
      if (result.success) {
        if (effectiveWindowId) {
          setEntityReturnDataWithToastSuppression(
            effectiveWindowId,
            { id: fornecedorId, nome: values.nome },
            'fornecedor',
            setWindowReturnData,
      ...
      toast.error(handleApiError(error, 'Erro ao atualizar fornecedor'))
    } finally {
      updateFormState(formId, (state) => ({
        ...state,
        isSubmitting: false,
