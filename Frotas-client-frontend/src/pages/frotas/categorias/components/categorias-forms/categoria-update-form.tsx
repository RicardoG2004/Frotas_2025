...
import { UpdateCategoriaDTO } from '@/types/dtos/frotas/categorias.dtos'
...
import { useUpdateCategoria } from '@/pages/frotas/categorias/queries/categorias-mutations'
...
interface CategoriaUpdateFormProps {
  modalClose: () => void
  categoriaId: string
  initialData: CategoriaFormSchemaType
  onSuccess?: (updatedCategoria: { id: string; designacao: string }) => void
...
const CategoriaUpdateForm = ({
...
}: CategoriaUpdateFormProps) => {
  const location = useLocation()
  const navigate = useNavigate()
  const windowId = useCurrentWindowId()
  const searchParams = new URLSearchParams(location.search)
  const instanceId = searchParams.get('instanceId') || 'default'
  const formId = `categoria-update-${instanceId}`
  ...
  useEffect(() => {
    const subscription = form.watch((value) => {
      ...
      setFormState(formId, {
        formData: value as CategoriaFormSchemaType,
        isDirty: hasChanges,
      ...
      if (effectiveWindowId) {
        ...
        const newTitle = value.designacao || 'Categoria'
  ...
  const onSubmit = async (values: CategoriaFormSchemaType) => {
    try {
      ...
      const requestData: UpdateCategoriaDTO = {
        designacao: values.designacao,
      }

      const response = await updateCategoriaMutation.mutateAsync({
        id: categoriaId,
        data: requestData,
      })
      const result = handleApiResponse(
        ...
        'Categoria atualizada com avisos'
      )

      if (result.success) {
        console.log('Categoria updated successfully:', result.data)

        // Set return data for parent window if this window was opened from another window
        if (effectiveWindowId) {
          setEntityReturnDataWithToastSuppression(
            effectiveWindowId,
            { id: categoriaId, designacao: values.designacao },
            'categoria',
            setWindowReturnData,
      ...
      toast.error(handleApiError(error, 'Erro ao atualizar categoria'))
    } finally {
      updateFormState(formId, (state) => ({
        ...state,
        isSubmitting: false,
        ...
        >
          ...
          <div className='flex flex-col justify-end space-y-2 pt-4 md:flex-row md:space-x-4 md:space-y-0'>
            ...
            <Button
              ...
              className='w-full md:w-auto'
            >
              {updateCategoriaMutation.isPending ? 'A atualizar...' : 'Atualizar'}
            </Button>
          </div>
        </form>
      </Form>
