...
import { useCreateManutencao } from '../../queries/manutencoes-mutations'
import { CreateManutencaoDTO } from '@/types/dtos/frotas/manutencoes.dtos'
...
interface ManutencaoCreateFormProps {
  modalClose: () => void
  onSuccess?: (newManutencao: { id: string }) => void
  shouldCloseWindow?: boolean
}

const ManutencaoCreateForm = ({
  modalClose,
  onSuccess,
  shouldCloseWindow = true,
}: ManutencaoCreateFormProps) => {
  const location = useLocation()
  const navigate = useNavigate()
  const windowId = useCurrentWindowId()
  const searchParams = new URLSearchParams(location.search)
  const instanceId = searchParams.get('instanceId') || 'default'
  const formId = `manutencao-${instanceId}`
  ...
  const { setFormState, updateFormState, hasFormData, resetFormState } =
    useFormsStore()
  const { removeWindow, updateWindowState, setWindowReturnData, setWindowHasFormData, findWindowByPathAndInstanceId } =
    useWindowsStore()

  const createManutencaoMutation = useCreateManutencao()
  ...
  const defaultValues: ManutencaoFormSchemaType = {
    dataRequisicao: null as any,
    fseId: '',
    funcionarioId: '',
    dataEntrada: null as any,
    horaEntrada: '',
    dataSaida: null as any,
    horaSaida: '',
    viaturaId: '',
    kmsRegistados: 0,
    totalSemIva: 0,
        ...
        timeoutId = setTimeout(() => {
          ...
          if (JSON.stringify(value) !== JSON.stringify(formData)) {
            ...
            if (effectiveWindowId) {
              ...
              const newTitle = 'Criar Serviço de Oficina'
              updateCreateWindowTitle(
                effectiveWindowId,
                newTitle,
                updateWindowState
  ...
  const onSubmit = async (values: ManutencaoFormSchemaType) => {
    try {
      ...
      const requestData: CreateManutencaoDTO = {
      ...
      const response = await createManutencaoMutation.mutateAsync(requestData)
      const result = handleApiResponse(
        response,
        'Serviço de Oficina criado com sucesso',
        'Erro ao criar Serviço de Oficina',
        'Serviço de Oficina criado com avisos'
      )
      ...
      toast.error(handleApiError(error, 'Erro ao criar Serviço de Oficina'))
    } finally {
      updateFormState(formId, (state) => ({
        ...state,
        isSubmitting: false,
      ...
      <Form {...form}>
        ...
        >
          ...
          <div className='flex flex-col justify-end space-y-2 pt-4 md:flex-row md:space-x-4 md:space-y-0'>
            ...
            <Button
              type='submit'
              disabled={createManutencaoMutation.isPending}
              className='w-full md:w-auto'
            >
              {createManutencaoMutation.isPending ? 'A criar...' : 'Criar'}
            </Button>
          </div>
        </form>
      </Form>
